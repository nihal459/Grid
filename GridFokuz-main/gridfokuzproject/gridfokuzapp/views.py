...
def AdminViewAllVendors(request):
    ...
    name = request.user
    vendors = AddVendors.objects.all().order_by('ventorcode', 'vendorname')  # Add the order_by method to sort by both fields
    is_admin = False
    user_groups = request.user.groups.values_list('name', flat=True)
    admin = list(user_groups)
    ...
    if request.method == "POST":
        ...
        if selected_vendors and category and sub_category:
            ...
            for lmt in products:
                ...
                if float(item.Vendor_Price) <= float(limit) and float(item.Vendor_Price) >= float(min_limit):
                    product.append(item)
                    print(product)
            for cat in product:
            ...
            if sorting != "null":
                if sorting == "acending":
                    for i in final:
                        ...
                        item_price_sort.append(items.Vendor_Price)
                    item_price_sort.sort()
                    for j in item_price_sort:
                        acend_items = AddProducts.objects.get(Vendor_Price=j)
...
def combo_products(request):
    ...
    if request.method == "POST":
        ...
        for i in combo_product:
            combo_prod = AddProducts.objects.get(id=i.product.id)
            product_price.append(combo_prod.Vendor_Price)
        combo_price = sum(product_price)
        if limit:
            for lmt in all_product:
                item = AddProducts.objects.get(id=lmt.id)
                if float(item.Vendor_Price) <= float(limit):
                    product.append(item)
                    # print(product)
            for i in product:
                com_product = AddProducts.objects.get(id=i.id)
                if float(com_product.Vendor_Price) < float(limit):
            ...
            while count > 0:
                ...
                for j in temp:
                    ...
                    temp_com_price.append(com_pro.Vendor_Price)
...
def combo_sort_products(request):
    ...
    vendors = AddVendors.objects.all()
    prod = AddProducts.objects.all().order_by('Category')
    price = [p.Vendor_Price for p in prod]
    limit = max(price)
    combo_product = ManualComboTemp.objects.filter(usr=request.user)
    product_price = []
    for i in combo_product:
        ...
        while count > 0:
            for i in range(10):
                ...
                for m in product_at_subcat:
                    combo.append(m)
                    total_price.append(m.Vendor_Price)
                    ...
                    for i in final_combo:
                        ...
                        for j in i:
                            total_combo_price.append(j.Vendor_Price)
            count = count-1
